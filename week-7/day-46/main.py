...
for song in song_title_text:
    result = spotify_conn.search(q=f"track:{song} year:{year}", type="track")
    print(result)
    try:
...
playlist = spotify_conn.user_playlist_create(user=user_id, name=f"{target_date} Billboard 100", public=False)
print(playlist)

spotify_conn.playlist_add_items(playlist_id=playlist["id"], items=song_uris)
